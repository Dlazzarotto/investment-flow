...
import { listarCarteira, listarProjetos, obterPapel, obterProjeto } from "@/lib/consultas";
...
export default async function ProjetoLayout({ children, params }:
  ...
  const [projetos, projeto, papel, carteira] = await Promise.all([
    listarProjetos(), obterProjeto(params.id), obterPapel(params.id), listarCarteira(),
  ]);
